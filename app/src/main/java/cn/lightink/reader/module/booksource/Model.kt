...
data class BookSourceJson(
    ...
    data class Search(
        ...
        val detailForPage: String = EMPTY
    )
    ...
    data class Detail(
        ...
        val update: String = EMPTY,
        val lastChapter: String = EMPTY,
        val catalog: String = EMPTY
    )
    ...
    data class Catalog(
        ...
        val orderBy: Int = 0,
        val name: String = EMPTY,
        val chapter: String = EMPTY,
        val booklet: Booklet? = null,
        val page: String = EMPTY
    )
    ...
    data class Auth(
        ...
        val cookie: String = EMPTY,
        val header: String = EMPTY,
        val params: String = EMPTY,
        val verify: String = EMPTY,
        val logged: String = EMPTY,
        val vip: String = EMPTY,
        val buy: String = EMPTY
    )
 ...
 * 换源搜索结果
 */
data class BookSourceSearchResponse(
    val book: DetailMetadata,
...
data class DetailMetadata(
    ...
    fun toMetadata() = MPMetadata(
        name,
        author,
        url,
        state = if (status.contains("完")) BOOK_STATE_END else BOOK_STATE_IDLE
    )
}
        ...
        fun build(expression: String): Query {
            ...
            return if (operators.isNotEmpty()) {
                ...
                operators.forEachIndexed { index, matchResult ->
                    val endIndex =
                        if (index < operators.lastIndex) operators[index + 1].range.first else expression.length
