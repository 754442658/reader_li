...
class BookSourceActivity : LifecycleActivity() {
    ...
    lateinit var mTopbar: TopbarView
    lateinit var mBookSourceNone: TextView
    lateinit var mBookSourceRecycler: RecyclerView

    fun initView() {
        mTopbar = findViewById(R.id.mTopbar)
        mBookSourceNone = findViewById(R.id.mBookSourceNone)
        mBookSourceRecycler = findViewById(R.id.mBookSourceRecycler)
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mBookSourceRecycler.addItemDecoration(
            VerticalDividerItemDecoration(
                this,
                R.dimen.margin_horizontal
            )
        )
    ...
    private fun showPopupMenu(position: Int, bookSource: BookSource) {
        ...
        BottomSelectorDialog(this, bookSource.name, items) { getString(it) }.callback { item ->
            when (item) {
                R.string.login -> startActivityForResult(
                    Intent(
                        this,
                        BookSourceAuthActivity::class.java
                    ).putExtra(INTENT_BOOK_SOURCE, bookSource.url), Activity.RESULT_FIRST_USER
                )
     ...
     * 构建数据适配器
     */
    private val adapter = PageListAdapter<BookSource>(
        R.layout.item_booksource,
        equalItem = { old, new -> old.url == new.url }) { item, bookSource ->
        ...
        if (bookSource.account) {
            controller.verify(bookSource).observe(this, Observer { verify ->
                ...
                item.view.mBookSourceAuth.backgroundTintList =
                    ColorStateList.valueOf(getColor(if (verify) R.color.colorAccent else R.color.colorGrapefruit))
            })
        }
        ...
        item.view.mBookSourceInstallLayout.setOnClickListener {
            showPopupMenu(
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        when {
            requestCode == Activity.RESULT_FIRST_USER && resultCode == Activity.RESULT_OK -> adapter.currentList?.indexOfFirst {
                it.url == data?.getStringExtra(
                    INTENT_BOOK_SOURCE
                ).orEmpty()
            }?.run { adapter.notifyItemChanged(this) }
        }
    }
}