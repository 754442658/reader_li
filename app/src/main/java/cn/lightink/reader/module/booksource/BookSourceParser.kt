...
class BookSourceParser(val bookSource: BookSource) {
    ...
    fun findDetail(metadata: SearchMetadata): DetailMetadata? {
        if (bookSource.type == "js") {
            ...
            if(metadata.detailForPage.isNotEmpty()){
                val response = BookSourceInterpreter.execute(metadata.detailForPage, bookSource.json.auth) ?: return null
                metadata.detail =findValue(response, bookSource.json.search.detail, true)
            }
...
//        val metadata = findDetailMetadata(book.link, response)
    ...
    private fun findSearchMetadata(response: BookSourceResponse) = SearchMetadata(
        ...
        detailForPage = findValue(response, bookSource.json.search.detailForPage, true)
    )
